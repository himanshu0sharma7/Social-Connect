...
public class sc_view extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sc_view);
                ...
                snapshot.getRef().child("ImageUriFromDatabase").addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            ...
                            Glide.with(sc_view.this).load(Uri.parse(imageModel.pipImageData)).into(setUserPipPhoto);
                        } else {
    ...
    private void setUserImagefuc(String uid) {
        userDataRef.child(uid).child("Profile_Image").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    User user = snapshot.getValue(User.class);
                    Glide.with(sc_view.this).load(user.User_Profile_Image_Uri).into(setUserImage);
                } else {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (like_dislike) {
                        if (snapshot.hasChild((FirebaseAuth.getInstance().getCurrentUser().getUid()))) {
                            ...
                            mp = MediaPlayer.create(sc_view.this, R.raw.heart_click_sound);
